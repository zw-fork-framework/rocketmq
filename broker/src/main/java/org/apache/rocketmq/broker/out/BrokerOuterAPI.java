...
public class BrokerOuterAPI {
    ...
    public List<RegisterBrokerResult> registerBrokerAll(
        ...
        List<String> nameServerAddressList = this.remotingClient.getAvailableNameSrvList();  // NameServer的注册地址
        if (nameServerAddressList != null && nameServerAddressList.size() > 0) {

            final RegisterBrokerRequestHeader requestHeader = new RegisterBrokerRequestHeader();  // 构建网络请求头
            requestHeader.setBrokerAddr(brokerAddr);
            requestHeader.setBrokerId(brokerId);
            requestHeader.setBrokerName(brokerName);
            requestHeader.setClusterName(clusterName);
            ...
            for (final String namesrvAddr : nameServerAddressList) {
                brokerOuterExecutor.execute(new AbstractBrokerRunnable(brokerIdentity) {  //自定义线程池
                    @Override
                    public void run0() {
                        try {
