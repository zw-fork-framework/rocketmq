...
public class NamesrvStartup {
    ...
    public static NamesrvController main0(String[] args) {
        try {
            parseCommandlineAndConfigFile(args);  // 构造NamesrvController初始化各种配置信息
    ...
    public static void parseCommandlineAndConfigFile(String[] args) throws Exception {
        ...
        namesrvConfig = new NamesrvConfig();  // NameServer业务参数
        nettyServerConfig = new NettyServerConfig();
        nettyClientConfig = new NettyClientConfig();  // NameServer网络参数
        nettyServerConfig.setListenPort(9876);
        if (commandLine.hasOption('c')) {
            String file = commandLine.getOptionValue('c');  // 通过-c命令指定配置文件的路径
            if (file != null) {
        ...
        MixAll.properties2Object(ServerUtil.commandLine2Properties(commandLine), namesrvConfig);  // 获取配置类容填充到namesrvConfig
        if (commandLine.hasOption('p')) {  // 打印当前加载的配置属性
    ...
    public static NamesrvController start(final NamesrvController controller) throws Exception {
        ...
        boolean initResult = controller.initialize(); //===> // 初始化核心逻辑
        if (!initResult) {
            controller.shutdown();
            System.exit(-3);
        }
