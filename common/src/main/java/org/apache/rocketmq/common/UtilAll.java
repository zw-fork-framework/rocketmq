...
public class UtilAll {
    ...
    public static String bytes2string(byte[] src) {  // byte 数组转十六进制字符串
        char[] hexChars = new char[src.length * 2];
        for (int j = 0; j < src.length; j++) {
            int v = src[j] & 0xFF;   // & 过之后，byte 转成 int
            hexChars[j * 2] = HEX_ARRAY[v >>> 4];   // 无符号右移 4 位，高位补 0 ，即取字节的高 4 位
            hexChars[j * 2 + 1] = HEX_ARRAY[v & 0x0F];   // 取字节低 4 位
        }
